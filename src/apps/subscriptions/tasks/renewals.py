...
class ChargeRenewalService(RecurringTaskService):
    def process_subscription(self, subscription: Subscription, tariff: Tariff) -> None:
        ...
        loki_logger.info(
            self.get_log_msg(
                f'Created payment {payment.id!r} for subscription {subscription.id!r} to renew subscription.'
            )
        )
        period_start = self.ensure_current_period_end(subscription)
        try:
            yoo_payment_response = self.try_charge_payment(
                payment, tariff, subscription, description=f'Продление подписки "{tariff.name}"'
            )
        except PaymentPendingRecurringError as e:
            loki_logger.warning(self.get_log_msg(f'Skipping renewal. {e.message}'))
    ...
    def act(self) -> int:
        subscriptions = Subscription.objects.get_renewals()
        count = 0
        for subscription in subscriptions:
            try:
                ...
                loki_logger.warning(self.get_log_msg(f'Skipping renewal. {e.message}'), exc_info=True)
                continue
...
@celery_app.task
def process_subscription_renewals() -> str:
    """
    Продлевает активные подписки с auto_renew=True.
    Запускается раз в CHECK_SUBSCRIPTION_PAYMENT_TIMEDELTA и обрабатывает подписки,
    у которых current_period_end <= now + CHECK_SUBSCRIPTION_PAYMENT_TIMEDELTA.
    """
    service = ChargeRenewalService()
    count = service()
    return service.get_log_msg(f'Charged {count} renewals.')
